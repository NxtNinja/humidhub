...
export function HourlyForecastCard({
  ...
  return (
    <div className="group relative min-w-24 md:min-w-28 flex flex-col items-center shrink-0 cursor-pointer p-4 rounded-2xl hover:bg-primary/5 transition-all duration-300 overflow-hidden border border-transparent hover:border-primary/20">
      ...
      <WeatherIcon
        code={weatherCode}
        className="absolute -right-4 -bottom-4 h-24 w-24 text-primary/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
      />

      <p className="font-mono text-[10px] sm:text-[11px] text-muted-foreground uppercase tracking-[0.2em] relative z-10 transition-colors group-hover:text-primary">
        {time}
      </p>
      ...
      <p className="font-heading text-4xl sm:text-5xl font-black leading-none relative z-10 text-foreground group-hover:text-primary transition-colors">
        {Math.round(temperature)}°
      </p>

      <p className="font-mono text-[9px] sm:text-[10px] text-center uppercase tracking-widest font-bold opacity-60 mt-3 relative z-10 line-clamp-2 w-full leading-tight">
        {weather}
      </p>
    </div>
  );
